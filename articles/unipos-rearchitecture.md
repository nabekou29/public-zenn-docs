...
topics: ["Elm", "React", "Next.js", "リアーキテクチャ"]
published: false
publication_name: "unipos"
---

Unipos では、約2年間にわたり Elm から React (Next.js) へのリアーキテクチャプロジェクトを実施しました。
そして、2025年3月 を持って完全に置き換えが終わりました。

今回の記事では、なぜリアーキテクチャをすることになったのか。
また、その中での個人的な想いや学びなどを振り返りたいと思います。

より詳細な技術的な話については別で記事を執筆しようと思っています。

:::message
文章中で「リアーキテクチャ」と書いている部分は、基本的に今回のプロジェクトのことを指しています。
ただし、今回のプロジェクトでは内部的な変更に留まらず、機能やデザインの整理もしたため、一般的な「リアーキテクチャ」とはずれるかもしれません。
:::
...
- 🏗️ **アーキテクチャの移行**
  ...
  - 新アーキテクチャ: **TypeScript + Next.js** (Page Router)
- 📦 **大まかなな内容**
  - Next.js を利用した新しい開発基盤・アーキテクチャの構築
  - 既存のページを Next.js のものにリプレイス
- 🔄 **置き換え方法**
...
私は、リアーキテクチャプロジェクト以前は、機能開発チームに所属し、既存の改修や新機能の開発をしていました。

開発にも慣れて、いくつかの機能の開発を経て、**「エンジニアの工数が原因で要件落としたりすること多いな…」** と思うようになりました。

実際に機能開発チームの中のコミュニケーションの中では以下のようなやり取りは珍しくなかったです。

機能開発の場:

> *Eng「今の仕様だとXX日かかってしまいそうです」
Des「それならこの機能は今回のスコープから落とそうか」*

社内や顧客からの FB を経て:

> *XXX「ここの機能を修正してほしい」
Eng 「けっこう時間かかりそうで、あまり費用対効果良くないですね」
　　　（あるいは）「実装が複雑になるので、できれば避けたいです」*
...
少なくとも、この時点で **開発速度を上げる改善** が必要だと強く感じていました。

### 🤝 リアーキテクチャすることへの合意

もともと、技術的負債が溜まっているよねという

## 😁 リアーキテクチャの目的を達成するためにやってよかったこと
...
これらは、消したり修正することで、スケジュールの短縮ができたり、実装もシンプルにすることができました。

### ⚡ 難しいところから攻める

ここでいう「難しいところ」とは、技術的負債が最も溜まっている箇所です。
...
*必要最低限の基盤を作ったあとは、一定のペースでリプレイスできると考えていた*
...
*改善アイテムを費用対効果で整理した際に、大掛かりなものは候補に乗らなかった*
...
Elm をやめたことは、開発組織のための前向きな決断で実際良かったとも思っているのですが、一方で個人的には少し寂しさもあります。
...
その文化を損ないまま、Reactの柔軟性やスピード感のある開発ができていけば良いなと思っています。
